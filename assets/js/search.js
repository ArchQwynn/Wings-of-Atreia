async function loadSearchIndex() {
  ...
  try {
    const res = await fetch(base + "assets/js/search-index.json?v=20260914-4", { cache: "no-store" });
...
(function initWoAUsabilitySuite() {
  ...
  function renderSavedList(entries, emptyText) {
    if (!entries.length) return `<p class="tool-empty">${escapeAttr(emptyText)}</p>`;
    return `<div class="tool-link-list">${entries.map(entry => `<a href="${escapeAttr(absoluteFromRelative(entry.url))}"><strong>${escapeAttr(entry.title)}</strong><small>${escapeAttr(entry.url.replace(/index\.html$/, ""))}</small></a>`).join("")}</div>`;
  }
  ...
  function addReferenceDrawer() {
    ...
    const refreshLists = () => {
      ...
      drawer.querySelector('[data-panel="recent"]').innerHTML = renderSavedList(readJson(STORAGE_RECENT), "No recently viewed pages yet.");
    };
    refreshLists();
